...
from .factorgraph import FactorGraphTrainerBase
from .nn import solver, util
...
class SatFactorGraphTrainer(FactorGraphTrainerBase):
    ...
    def _check_recurrence_termination(self, active, prediction, sat_problem):
        ...
        active_ = active.clone().to(torch.bool)
        ...
        if sat_problem._batch_replication > 1:
            ...
            active[active_[:, 0], 0] = (dup_batch[active_[:, 0], 0] > 0)
        else:
            active[active_[:, 0], 0] = (output[active_[:, 0], 0] <= 0.5).to(torch.uint8)
        ...
        # else:
        #     active[active[:, 0], 0] = (output[active[:, 0], 0] <= 0.5).to(torch.uint8)
