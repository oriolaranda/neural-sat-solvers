...
class SatLossEvaluator(nn.Module):
    ...
    def forward(self, variable_prediction, label, graph_map, batch_variable_map, 
        ...
        # Global step is the total number of steps, epoch = steps*batch_size
        # self._alpha = exploration = 0.1
        # max_coeff = 10

        # Mask*variable_prediction
        #print("\n", "\n", "DEBUG")
        #print("graph map:", graph_map)
        #print("variable_pred:", variable_prediction.shape, "\nglobal_step:", global_step.shape, "\nalpha:", self._alpha)

        # coeff = global_step**alpha = global_step*0.1 -> slow grow, the max_coeff is 10
        coeff = torch.min(global_step.pow(self._alpha), torch.tensor([max_coeff], device=self._device))
        # self._logger.log({"coeff": coeff})
        #print("coeff:", coeff.shape)
        # wandb.log({"coeff": coeff})
        ...
        #print("mask:", signed_variable_mask_transpose.shape,"\nfunction_mask:", function_mask.shape)
        edge_values = torch.mm(signed_variable_mask_transpose, variable_prediction)
        #print("edge_values:", edge_values.shape)
        edge_values = edge_values + (1 - edge_feature) / 2
        #print("edge_features:", edge_feature.shape, "\nedge_values_after:", edge_values.shape)
        ...
        #print("clause_value:", clause_value.shape)
        clause_value = 1 + (clause_value - 1).pow(loss_sharpness)
        # wandb.log({"k": loss_sharpness})
        #print("clause_value_after:", clause_value.shape)
        loss = torch.mean(SatLossEvaluator.safe_log(clause_value, eps))
        #print("loss:", loss)
        return loss
