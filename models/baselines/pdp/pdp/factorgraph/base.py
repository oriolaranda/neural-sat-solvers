...
from .dataset import FactorGraphDataset
...
class FactorGraphTrainerBase:
    ...
    def _train_epoch(self, train_loader, optimizer):
        ...
        for (j, data) in enumerate(train_loader, 1):
            ...
            for i in range(segment_num):
                ...
                total_example_num += (batch_variable_map.max() + 1).item()
    ...
    def _train_batch(self, total_loss, optimizer, graph_map, batch_variable_map, batch_function_map, 
        ...
        for (i, model) in enumerate(self._model_list):
            ...
            # im = prediction[0].detach().cpu()
            # ax = sns.heatmap(im, vmin=0, vmax=1, cmap='viridis', annot=True)
            # wandb.log({"train_out": wandb.Image(ax)})
            # plt.close()
            for s in state:
                del s
    ...
    def _test_batch(self, error, graph_map, batch_variable_map, batch_function_map, 
        ...
        for (i, model) in enumerate(self._model_list):
            ...
            b_ = this_batch_size.float() #TODO: why this scaling factor?
            a_ = self._compute_evaluation_metrics(
            ...
            error[:, i] += (b_ * a_).detach().cpu().numpy()
            # im = prediction[0].detach().cpu()
            # ax = sns.heatmap(im, vmin=0, vmax=1, cmap='viridis', annot=True)
            # wandb.log({"test_out": wandb.Image(ax)})
            # plt.close()
            for p in prediction:
                del p
    ...
    def train(self, train_list, validation_list, optimizer, last_export_path_base=None,
        ...
        unique_id = f'{self._config["exploration"]}-{self._config["learning_rate"]}-{datetime.now().strftime("%d-%m--%H-%M")}'
        # wandb.init(project=self._config['model_name'], config=self._config, name=unique_id, id=unique_id)
        
        if self._use_cuda:
            torch.backends.cudnn.benchmark = True

        # wandb.watch(self._model_list[0])
        ...
        for rep in range(self._config['repetition_num']):
            ...
            for epoch in range(self._config['epoch_num']):
                ...
                # wandb.log({"train_loss": losses[i, epoch, rep], "val_acc": 1 - errors[0, i, epoch, rep].flatten(), "epoch": epoch})
    ...
    def test(self, test_list, import_path_base=None, batch_replication=1):
        ...
        for file in test_files:
            ...
            if self._config['verbose']:
                ...
                self._logger.info('Time spent: %s seconds' % duration)
            # wandb.log({f"test_acc/{file}": 1 - error[0, 0].flatten()})
