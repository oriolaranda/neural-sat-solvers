...
def is_sat(n, clause_list):
    ## Note: Invoke your SAT solver of choice here for generating labeled data.
    clauses = [[int(x_i) for x_i in x] for x in clause_list]
    with Solver(name='m22', bootstrap_with=clauses, use_timer=True) as solver:
        return int(bool(solver.solve())), solver.get_model()
...
class CNFGeneratorBase(object):
    ...
    def generate_dataset(self, size, output_dimacs_path, json_output, name, sat_only=True, train=False):
        max_trial = 50
        ...
        if train:
            self._alpha_resolution = 1
            self._alpha_inc = self._max_alpha - self._min_alpha
        
        for j in range(self._alpha_resolution):
            postfix = ''
            if not train:
                postfix = '_' + str(j) + '_' + str(self._alpha)
            ...
            with open(json_output + postfix + ".json", 'w') as f:
                for i in range(size):
                    ...
                    for _ in range(max_trial):
                        n, m, graph_map, edge_feature, _, label, solution, clause_list = self.generate_complete()
                    ...
                    if flag:
                        ...
                        file_name = 'dimacs_' + str(i) + '_sat=' + str(label) 
                        with open(os.path.join(output_dimacs_path + postfix, f"{file_name}.DIMACS"), 'w') as g:
                            g.write(self._to_dimacs(n, m, clause_list) + '\n')
                        
                        if label:
                            sol_file_name = f"{file_name.split('sat=')[0]}sol.json"
                            with open(os.path.join(output_dimacs_path + postfix, sol_file_name), 'w') as h:
                                json.dump(solution, h)
...
class UniformCNFGenerator(CNFGeneratorBase):
    ...
    def generate_complete(self):
        ...
        label, sol = is_sat(n, clause_list)
        return n, m, graph_map, edge_features, None, label, sol, clause_list
...
class ModularCNFGenerator(CNFGeneratorBase):
    ...
    def generate_complete(self):
        ...
        q = np.random.uniform(self._min_q, self._max_q)
        _c = np.random.randint(self._min_c, self._max_c + 1)
        # c = max(self._k + 1, min(_c, int(n / self._k)))
        c = min(int(n/self._k), max(_c, self._k))
        size = int(n / c)
        community_size = size * np.ones(c, dtype=np.int32)
        community_size[c - 1] += (n - np.sum(community_size))
        # print("n:",n,"k:", self._k, "n/k:", n/self._k, "_c:", _c, "c:", c, size, community_size)
        p = q + 1.0 / c
        edge_num = m * self._k
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--train', dest='train', help='Train dataset. All instances written in same output dir', required=False, 
    ...
    generator.generate_dataset(args['size'], args['out_dir'], args['out_json'], args['name'], args['sat_only'], args['train'])